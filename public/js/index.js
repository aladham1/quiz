...
function restoreExam() {
    ...
    function exam_icon(res) {
        if (res != null) {
            ...
            $('#blah').on('load', function () { URL.revokeObjectURL(img) });
            $('#blah').attr('src', img);
        } else {
            $('.imgInp_hidden').val('Exam_icon');
    ...
    function exam_reward_image(res) {
        if (res != null) {
            var img = URL.createObjectURL(res);
            $('#showReward').on('load', function () { URL.revokeObjectURL(img) });
...
function populateDB(exam, intro, exam_questions, data_copy_with_urls) {
    ...
    for ( var key in intro ) {
        ...
        data_copy_with_urls[DBkeyName] = {'data': isJSON(data_copy_with_urls['Intro'][key]) || data_copy_with_urls['Intro'][key], 'o': key.split('_')[0]};
...
function renderSubjectListHTML(type, text, id = null, sort = true) {
    ...
    if (type == 'title') {
        title = 'Title'; iconName = 'text'; htmlBlock = '<div class="drgT2 center title_data">'+text+'</div>';
    }else if (type == 'video') {
        title = 'Video'; iconName = 'video'; htmlBlock = '<div class="drgvdo center video_data">'+
    ...
    '</div>';
        ...
        title = 'Audio'; iconName = 'audio';
         htmlBlock = '<div class="drgAudio center">'+'<audio controls>'+'<source onload="URL.revokeObjectURL(this.src);console.log(\'revoked\')" type="audio/wav">'+'</audio>'+'</div>'; editEvent = '';
    }else if (type == 'image') {
        src = (text instanceof Blob || text instanceof File) ? URL.createObjectURL(text) : text;
        title = 'Image'; iconName = 'img';
         htmlBlock = '<div class="drgimg center">'+'<img onload="URL.revokeObjectURL(this.src);console.log(\'revoked\')"/>'+'</div>'; editEvent = '';
    }else if (type == 'paragraph') {
        title = 'Paragraph'; iconName = 'note'; htmlBlock = '<div class="drgtxtp  paragraph_data">'+text+'</div>';
    }else if (type == 'table') {
        title = 'Table'; iconName = 'table'; htmlBlock = '<div class="drgvdo center">'+text+'</div>';
    }else if (type == 'file') {
        var file_name = (text instanceof Blob || text instanceof File) ? text.name : text;
        title = 'File'; iconName = 'attach_white'; htmlBlock = '<div class="drgT2 center title_data">'+file_name+'</div>'; editEvent = '';
    }else if (type == 'order') {
        title = 'Order Button'; iconName = 'order-white'; htmlBlock = '<div class="drgT2 center title_data">'+text+'</div>';
    }
...
function finishDataPopIntroSave(type, cls, data, render = true, DBid = null) {
    showLoader();
    ...
    .then(
        function(res){
            console.log(res);
            render == true ? renderSubjectListHTML(type, is_file ? res : res['data']) : false;
            hideLoader();
            $(cls).val('');
            recorded_audio = null;
            $('#mask').trigger("click");
...
function saveDataPopIntro(type,index='', render = true){
    var audio_name = localStorage.getItem("intro_draft_audio_name");
    if(type=='title'){
        ...
        if( d_exam_id ==null){
            ...
            if(recorded_audio==null){
                ...
                finishDataPopIntroSave(type, null, recorded_audio, render, index);
                localStorage.removeItem("intro_draft_audio_name");
            }
        }
    }else if(type=='image'){
        ...
        var content = paragraph_editor.getData(); console.log(content);
        $('.intro_paragraph').val('');
        $('#mask').click();
        var d_exam_id = localStorage.getItem("draft_exam_id"); console.log(d_exam_id);
        if( d_exam_id ==null){
...
function renderQuestionsListHTML(type, text, id = null, sort = true) {
    ...
    if (type == 'wg') {
        title = 'Words Game'; param = 'word_game'; iconClass = 'wrdgmicon'; txtClass = 'wgQTitle'; DBkey = 'WordGame'+count;
    }
    else if (type == 'dd') {
        title = 'Drag & Drop'; param = 'dd'; iconClass = 'ddicon'; txtClass = 'ddTxt'; DBkey = 'Puzzle'+count;
    }
    else if (type == 'mcq') {
        title = 'Multiple Choice'; param = 'mc'; iconClass = 'mcicon'; txtClass = 'mcqTxt'; DBkey = 'MultipleChoiceQuestion'+count;
    }
    else if (type == 'project') {
        title = 'Project'; iconClass = 'prjicon'; param = 'project'; txtClass = 'projcTitle'; DBkey = 'Project'+count;
    }
...
function tmpChecker(id, store) {
    ...
    .then( function(keys) {
        for (var i = 0; i < keys.length; i++) {
            if(keys[i].indexOf(id) != -1 && keys[i].indexOf('_tmp') != -1) { promises.push(processTmp(keys[i], store)); }
        }
        return Promise.all(promises);
    })
}
            ...
            .then(
                function() {
                    ...
                    for (var x = 1; x < 5; x++) {
                        ...
                        //remove html prview
                        $('.ansImgArea_'+x).html(''); $('.ansImgArea_'+x).hide();
                        $('.ansAud_'+x).html(''); $('.ansAud_'+x).hide();
                    }
...
function publishExam(draft = false){
    ...
    .then(function(orders_and_keys) {
        ...
        .then( function (res) {
            ...
            axios.post(submitURL, exam_form, { headers: {'content-type': 'multipart/form-data'},
...
function submitProjectByStudent(submitURL, no_questions, examURL){
    ...
    .then( function() {
        return axios.post(submitURL, submission_form, { headers: {'content-type': 'multipart/form-data'},
            onUploadProgress: function (progressEvent) {
                //TODO: add upload bar
            }
        })
...
function showHistryReward(url){
    ...
    .then(function (rs){ console.log(rs);
        ...
        if(reward_type==0){
            ...
            span.innerHTML = renderCert({lang: reward_data.lang, sponser: reward_data.sponser, name: reward_data.user_name, creation_date: reward_data.creation_date, creation_time: reward_data.creation_time, exam_title: reward_data.exam_title, exam_owner: reward_data.exam_owner, cert_id: reward_data.cert_id});
        }
        $('.rewMsk').fadeIn();
        showLoader();
...
function openComment(url){ console.log("clicking");
    showLoader();
    axios.get(url)
    .then(function(rs){
