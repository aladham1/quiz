...
function saveProject(id){
    ...
    if( exam_id ==null){
        ...
        .then( function () {
            questions.setItem(jsonId+'_files', {image:image,audio:audio,link:video})
        })
        .then( function () {
        ...
        /*
        $.post("saveExamProjects",{examid:exam_id,project:project,project_image:image,audio:audio,link:video},function(rs){ console.log(rs);
            if(rs!=0){
...
function saveProjectEdit(){
    ...
    if( exam_id ==null){
        ...
        $.post(base_url+"saveExamProjectsEdit",{examid:exam_id,project:project,project_image:image,audio:audio,link:video},function(rs){ console.log(rs);
            if(rs!=0){

                renderQuestionsListHTML('project', project);

                //var obj = $.parseJSON(rs);
                //var html = '<div class="dragBx prjctItm" data="'+obj.id+'">'+
                //                '<div class="drgHdr">'+
                //                    '<div class="drgAction">'+
                //                        '<span class="drgDel" onclick="deleteProjectEdit('+obj.id+')">DELETE</span>'+
                //                        '<span class="drgEdt" onclick="editProjectEdit('+obj.id+')">EDIT</span>'+
                //                    '</div>'+
                //                    '<div class="drgT1">'+
                //                        '<span>Project</span>'+
                //                    '</div>'+
                ...
                //                    '</div>'+
                //                '</div>'+
                //                '<div class="drgWhte2"> <span class="prjicon">Project Icon</span>'+
                //                    '<div class="drgT3 projcTitle center">'+project+'</div>'+
                //                '</div>'+
                //            '</div>';
...
function editProject(id){
    $.post("getExamProject",{id:id},function(rs){
        var obj = $.parseJSON(rs);
        $('.project_title').val(obj.project);
        
        if(obj.project_image!='' && obj.project_image!=null){
            $('.quest_image').val(obj.project_image);

            var split = obj.project_image.split(',');
            var html = '';
            ...
            var html = '<audio controls>'+
                         '<source src="api/uploads/'+obj.audio+'.wav" type="audio/wav">'+
                        '</audio>';
            $('.praudTag').html(html);
            $('.prAudBx').show();
...
function editProjectEdit(id){
    $.post(base_url+"getExamProjectEdit",{id:id},function(rs){
        var obj = $.parseJSON(rs);
        $('.project_title').val(obj.project);
        ...
        $('.pr_svBtn').attr("onclick","updateProjectEdit("+obj.id+")");
...
function updateProject(id){
    ...
    var video = $('.quest_video').val();
    var project = $('.project_title').val();
    $.post("updateProject",{id:id,project_image:image,audio:audio,link:video},function(rs){
        if(rs==1){
            $('.prjctItm').each(function(){
...
function updateProjectEdit(id){
    ...
    $.post(base_url+"updateProjectEdit",{id:id,project:project,project_image:image,Qtype:Qtype,audio:audio,link:video},function(rs){
        if(rs==1){
            $('.prjctItm').each(function(){
                if($(this).attr('data')==id){
                    $(this).find(".projcTitle").text(project);
...
function deleteProject(id){
    swal({
        ...
        if (isConfirm) {
            $.post("deleteProject",{id:id},function(rs){
                if(rs==1){
                    $('.prjctItm').each(function(){
                        if($(this).attr("data")==id){
                            $(this).remove();
...
function deleteProjectEdit(id){
    swal({
        ...
        if (isConfirm) {
            $.post(base_url+"deleteProjectEdit",{id:id},function(rs){
                if(rs==1){
                    $('.prjctItm').each(function(){
                        if($(this).attr("data")==id){
                            $(this).remove();
