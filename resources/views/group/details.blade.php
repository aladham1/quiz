...
@section('content')
    ...
    <section class="pgTop">
        <div class="pfVew">
            <div class="pfbx1"
                 style="background:url('@if ($group->image != null || $group->image != ''){{ url(Storage::url($group->image)) }}@else{{ url('images/placeholder.jpeg') }}@endif') no-repeat center center;background-size: cover;"></div>
            <div class="pfbx2">
                <div class="pfbx3">
                ...
                <div class="pfbx82">
                    {{ substr($group->description,0,50) }}...<span class="rdmre"
                                                                   onclick="showPopGropInfo('{{ route('groups.showDesc', ['group' => $group->id]) }}')">Read More</a>
                </div>
                <div class="pfbx7">
                    <div class="pfbx92">
                        <span class="p9Shrp"
                              onclick="shareData('{{ $group->title }}','{{ $group->title }} - {{ $owner->name }}','{{ route('groups.show', ['group' => $group->id]) }}')">Share</span>
        ...
        <div class="ptBox2">
            <ul class="ulicn2" style="display:inline-flex">
                <li class="actv" onclick="showTab(this,'quest')"><span class="icnm icni"></span><span class="icnTxt">Quests</span><span
                        class="nmbr">{{ $group->exams_count }}</span></li>
                <li onclick="showTab(this,'follower')"><span class="icnflw icni"></span><span
                        class="icnTxt">Followers</span> <span class="nmbr">{{ $group->followers_count }}</span></li>
                <li onclick="showTab(this,'news')"><span class="icnflw icni"
                                                         style="background:url('<?=$base_url?>images/msg_icon.svg') no-repeat center center"></span><span
                        class="icnTxt">News</span> <span class="nmbr">{{ $group->news_count }}</span></li>
            </ul>
        </div>
    </section>
            ...
            <ul class="follwer_list">
                @foreach ($followers as $follower)
                    <li>
                        ...
                        <div class="pfbx3">
                            <div class="pfbx4"><img
                                    src="{{ isset($follower->avatar) ? url(Storage::url($follower->avatar)) : url('images/user.svg') }}">
                            </div>
                            ...
                            <div class="qsRow1 q4c">
                                ...
                                <div class="clBtn">
                                    <aside class="setRnew q4d">
                                        <span class="btnIc"
                                              onclick="shareData('{{ $item->title }}','{{ $item->title }} - {{ $item->body }}','{{ $item->link }}')"><img
                                                src="<?=$base_url?>images/share_v.svg"></span>
                                    </aside>
                                    <div class="magic"></div>
                                </div>
                            </div>
                        ...
                        <div class="nhlBx">
                            ...
                            <div class="nhRw1">
                                ...
                                <a href="{{ route('profile', ['id' => $exam->owner->id]) }}" style="color:#000">
                                    <div class="nhrImg2"><img
                                            src="{{ isset($exam->owner->avatar) ? url(Storage::url($exam->owner->avatar)) : url('images/user.svg') }}"/>
                                    </div>
                            ...
                            @php
                                $examSolved =(int)$exam->have_preq_exam - 1000;
                                $result = auth()->user()->solved()
                            ->wherePivot('exam_id', $examSolved)->first();
                            @endphp
                            <div class="qsRow1 q4c">
                                @if (!$result || $result->analysis->percentage <= $exam->pass_percentage)
                                    <a href="#"> <span class="clqBx"></span>
                                    </a>
                                @else
